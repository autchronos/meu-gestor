...
-- Grava só como si mesmo (user_id) E só num negocio do qual e membro (ou sem negocio).
CREATE POLICY suporte_insere ON suporte FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid() AND (negocio_id IS NULL OR e_membro(negocio_id)));
