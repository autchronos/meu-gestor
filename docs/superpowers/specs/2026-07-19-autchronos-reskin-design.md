...
3. **Refinamentos de componente** (do `dashboard-referencia`): faixa/hero navy com
   o **valor do saldo inteiro em dourado**, labels UPPERCASE com `tracking`,
   divisórias finas (`rule`), caixas de ícone quadradas nas linhas de extrato,
...
- **Hero/saldo navy:** bloco `bg-marca text-white`; rótulo UPPERCASE
  `tracking-[0.18em] text-white/70` com um chip dourado; valor **inteiro em
  `text-dourado`** (`font-serif text-4xl/5xl tabular-nums`) — dourado sobre navy
  tem contraste alto, então o saldo todo fica dourado (verde/vinho seguem
  exclusivos de entrada/saída). → componente `ValorDestaque({ valor })`.
